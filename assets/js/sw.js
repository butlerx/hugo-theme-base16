...
function cleanupLegacyCache() {
  ...
  return new Promise((resolve, reject) => {
    caches
      .keys()
      .then(
        keys => (legacyKeys = keys.filter(key => !~currentCaches.indexOf(key))),
      )
      .then(legacy => {
        if (legacy.length) {
...
self.addEventListener('fetch', event => {
  event.respondWith(
    caches.open(CACHE_VERSIONS.content).then(cache => cache
      .match(event.request)
      .then(response => {
        if (response) {
          ...
          if (date) {
            ...
            if (ttl && age > ttl) {
              return new Promise(resolve => fetch(event.request.clone())
      ...
      .then(response => {
        ...
        return fetch(event.request.clone())
          .then(response => {
            ...
            return caches
              .open(CACHE_VERSIONS.notFound)
              .then(cache => cache.match(NOT_FOUND_PAGE));
          })
          .then(response => {
            if (response) {
              return response;
